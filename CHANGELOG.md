...
|------|---------|---------|
| 2026-09-11 | 1 | feat: A `startup` event is reported to trace, off the main thread and with an opt-out (PR #573) — Roam now reports that it is being played to [trace](https://github.com/Stephenson-Software/trace) at `https://trace.danielstephenson.dev`: one `startup` event per launch and one `world-loaded` event per save opened, each tagged with the version from `version.txt` and nothing else. The client is `src/lib/trace_client.py`, vendored byte-for-byte from trace-client-python apart from a docstring note (standard library only; a single daemon sender thread, never raises, bounded queue of 256), with its upstream test at `tests/lib/test_trace_client.py` and a `pyproject.toml` `[tool.black] force-exclude` so `./format.sh` and CI leave both as vendored. Three flat camelCase settings were added to `Config` — `usageReportingEnabled` (default `true`), `usageReportingEndpoint`, `usageReportingKey` — with the defaults built in so a user `config.yml` seeded before this change still reports. The first launch with reporting active logs one INFO line naming what is sent and the opt-out, then writes `usageReportingEnabled: true` so it is not shown again. `main()` builds the client after `Config()`, `Roam` takes it as an optional constructor argument and defaults to `TraceClient.disabled()`, so the Pyodide entry point (which has no threads or sockets) and the unit tests report nothing; `sys.platform == "emscripten"` is guarded independently, and `ROAM_USAGE_REPORTING=0` forces reporting off for a process, which the `TextPlaythrough` harness sets so a test run is never counted as a player. `--selftest` returns before the client exists. +39 tests; 1330 passed locally |
...
### 2026-09-11 — Usage reporting to trace, with an opt-out and a first-run notice (PR #573)
- **Context:** part of a rollout wiring every Preponderous-Software program into the trace usage service, following the Dans-Plugins plugin rollout documented in `Stephenson-Software/trace` → `docs/CONSUMERS.md`. The program key was issued for the `application` value `roam` exactly, and was copied into `src/config/config.py` by script from the key file rather than retyped (a hand-copied key was the cause of an outage in the earlier rollout); `grep -c` of the key file's contents against `config.py` prints 1.
- **What is sent:** `startup` once per launch and `world-loaded` from `Roam.initializeWorldScreen()` (the save-selection callback, i.e. a save was created or opened), each with a single tag `version` = `Config.getVersion()` (the bundled `version.txt`). Never a username, hostname, IP, path, save name or anything else identifying a person or machine. `--selftest` returns before `Config()` and never builds a client, so a packaging check in CI is not counted as a player.
- **Where the client lives:** `src/lib/trace_client.py`, next to the other vendored libraries, unmodified from upstream apart from the docstring line `Vendored into roam unmodified apart from this note.`; `tests/lib/test_trace_client.py` is the upstream test with only its two import lines changed. Both are `force-exclude`d from black in a new `pyproject.toml` (black reads it from the repo root; `pytest.ini` still wins for pytest) because black 23.1.0 would reformat the upstream file and the point of vendoring byte-for-byte is being able to diff it later.
- **Settings:** `Config` is a flat `key: value` parser, so the three settings are flat camelCase like every other key — `usageReportingEnabled` (bool, default `true`), `usageReportingEndpoint` (default `https://trace.danielstephenson.dev`), `usageReportingKey` (default built in). The defaults live in code, not only in the shipped `config.yml`, for the same reason the plugin rollout read Bukkit config with one-argument getters: a Windows/macOS user config seeded from an older bundled `config.yml` has none of these keys and must still report. `config.yml` documents the block in comments and deliberately does not set `usageReportingEnabled`, so every installation — from source, frozen, or seeded — sees the notice once.
- **First-run notice:** `usageReporting.showFirstRunNotice()` logs `Usage reporting is on: roam sends a startup event and a world-loaded event (program name and version only) to trace.danielstephenson.dev. Turn it off with usageReportingEnabled: false in config.yml.` on the game's normal structlog channel (stderr in the graphical frontend, `roam.log` in text mode) when reporting is active and the file has no `usageReportingEnabled` key, then `Config.acknowledgeUsageReporting()` writes the key through the existing `_writeKeyValues` so the presence of the key is the marker. No dialog. The notice names both events rather than the rollout template's "a startup event" because Roam sends two.
- **Browser build:** the Pyodide build runs these same sources under Emscripten, which has no OS threads or sockets. `web/pyodide_main.py` constructs `Roam` without a client, so it keeps the default `TraceClient.disabled()`, and `usageReporting.isBrowserBuild()` (`sys.platform == "emscripten"`) independently refuses to build an enabled client there. Only the desktop build reports.
- **Never from tests:** `ROAM_USAGE_REPORTING=0` (also `false`/`off`/`no`) forces reporting off for a process; `tests/integration/textPlaythrough.py` sets it on the child it launches, so the playthroughs and every `.roamscript` neither report nor write the first-run marker into the repo's `config.yml`. Every enabled client in `tests/test_usageReporting.py` points at a loopback `ThreadingHTTPServer` stub.
- **Local stub proof:** the real game was run twice under the `TextPlaythrough` pty driver with `usageReportingEndpoint` pointed at a loopback stub. Run 1 (boot → create save → world): the stub received `{"application":"roam","name":"startup","tags":{"version":"0.13.0-SNAPSHOT-8-8-2026"}}` then `{"application":"roam","name":"world-loaded","tags":{"version":"0.13.0-SNAPSHOT-8-8-2026"}}` on `/api/metrics` with the bearer key, the notice appeared once in the log, and `config.yml` gained `usageReportingEnabled: true`. Run 2 (boot → quit): one more `startup`, zero notice lines. Production was never contacted.
- **Tests (+39):** 11 vendored client tests; 5 in `tests/config/test_config.py` (defaults, opt-out and overrides parsed, presence counts as acknowledged, acknowledge writes the key once and a second `Config` sees it, acknowledge preserves an opt-out); 23 in `tests/test_usageReporting.py` (application name, enabled by default, opt-out, six env-override spellings, four non-off values leave config in charge, browser build starts no thread, bad endpoint yields a disabled client without raising, version tag source, the exact `startup` and `world-loaded` bodies at a stub, a `Roam` built without a client sends nothing, the notice logged once then acknowledged, its wording, not shown when off/env/browser, and a round trip through a real config file).
- **Validation:** 1330 passed via `SDL_VIDEODRIVER=dummy SDL_AUDIODRIVER=dummy python3 -m pytest -q --ignore=tests/mapimage/test_mapImageGenerator.py` (up from 1291; that file needs Python 3.10+ syntax the sandbox's 3.8 cannot collect, pre-existing, and CI runs 3.12 with no exclusion). `python3 -m black --check src tests` clean on 23.1.0. `web/build_zip.py` confirmed to bundle `src/lib/trace_client.py` and `src/usageReporting.py`. PyInstaller is not installed in the sandbox, so the frozen build is unverified here; `lib.trace_client` is imported the same way as the already-frozen `lib.graphik`.
- **Held from auto-merge:** `config.yml` and `src/lib/**` are both on the do-not-auto-merge path list, so this PR is left for the codeowner.
